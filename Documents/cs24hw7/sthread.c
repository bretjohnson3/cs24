...
void sthread_start(int timer) {
    /* We need the timer and scheduler to happen atomically so we lock */
    __sthread_lock();
    if (timer)
        start_timer();
    ...
    // We are done with the operations so we unlock
    __sthread_unlock();
}
 ...
 */
void __sthread_finish(void) {
    /* We lock here since the function will eventually call __sthread_switch
    and we want it to lock before that and before it changes anything else */
    __sthread_lock();
 ...
 */
void sthread_yield() {
    /* We need to lock since this function calls __sthread_switch() which
    calls the scheduler */
    __sthread_lock();
    __sthread_switch();
 ...
 */
void sthread_block() {
    /* We want to lock so that this functions operations and the scheduler
    operations happen atomically so we lock */
    __sthread_lock();
...
void sthread_unblock(Thread *threadp) {
    ...
    /* We lock so that shared resources (the buffers) are updated and used
    atomically */
    __sthread_lock();
