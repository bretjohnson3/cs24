 ...
 */
addr_t get_offset_in_block(cache_t *p_cache, addr_t address) {
    int offset = p_cache->block_offset_bits;
    int mask = (1 << offset) - 1;
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    int o_bits = p_cache->block_offset_bits;

    *offset = get_offset_in_block(p_cache, address);

    int s = p_cache->sets_addr_bits;
    int mask = p_cache->num_sets - 1;
    *set = mask & (address >> o_bits);

    *tag = address >> (o_bits + s);
}
 ...
 */
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
    return address - get_offset_in_block(p_cache, address);
}
...
cacheline_t * choose_victim(cacheset_t *p_set) {
    ...
    for (int i = 0; i < p_set->num_lines; i++)
    {
        if (!p_set->cache_lines[i].valid)
        {
            return &(p_set->cache_lines[i]);
        }
        if (p_set->cache_lines[i].last_access < i_victim)
        {
            i_victim = p_set->cache_lines[i].last_access;
