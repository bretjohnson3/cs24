...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
    ...
    if (context != NULL)
    {
        ...
        if (current->state == ThreadReady || current->state == ThreadBlocked)
        {
    ...
    if (current == NULL)
    {
        // If no threads in blocked queue then we have handled all threads
        if (queue_empty(&blocked_queue))
        {
            printf("Success! All threads have been completed\n");
            exit(0);
        }
        // otherwise, we are in a deadlock state
        else
        {
            printf("Deadlock! Threads still in blocked queue\n");
        }
    }
...
Thread * sthread_create(void (*f)(void *arg), void *arg) {
    // Memory allocations and checks for stack pointer and thread
    void * stack = malloc(DEFAULT_STACKSIZE);
    if (stack == NULL)
    {
        printf("Allocation failed for stack!\n");
        exit(1);
    }
    Thread *thread = malloc(sizeof(Thread));
    if (thread == NULL)
    {
        printf("Allocation failed for thread!\n");
        exit(1);
    }
    // Initialize thread and its memebers
    thread->context = __sthread_initialize_context((char *) (stack) +
