...
float Shape_getMass(Shape_Data *this) {
    ...
    float v = this->class->getVolume(this);
    return v * d;
}
...
Box_Data * new_Box(float L, float W, float H, float D) {
    Box_Data* b = malloc(sizeof(Box_Data));
    Box_init(b, &Box, L, W, H, D);
    return b;
}
...
float Box_getVolume(Box_Data *this) {
    float v = this->length * this->width * this->height;
    return v;
}
...
Sphere_Data * new_Sphere(float R, float D) {
    Sphere_Data * s = malloc(sizeof(Sphere_Data));
    Sphere_init(s, &Sphere, R, D);
    return s;
}
...
float Sphere_getVolume(Sphere_Data *this) {
    float pi = 3.141592;
    float v = (4.0 / 3.0) * pi * this->radius * this->radius * this->radius;
    return v;
}
...
Cone_Data * new_Cone(float BR, float H, float D) {
    Cone_Data * c = malloc(sizeof(Cone_Data));
    Cone_init(c, &Cone, BR, H, D);
    return c;
}
...
float Cone_getVolume(Cone_Data *this) {
    float pi = 3.141592;
    return (1.0 / 3.0) * pi * this->base_radius * this->base_radius *
     this->height;
}
