...
/* This helper function will get called by foreach_global so that we mark all
  * the global variables as true. Eventually, we will sweep everything marked
  * as false
  */
...
void mark(const char *name, Reference ref){
    ...
    Value * val = deref(ref);
    ValueType val_type = val->type;

    // Cases for the type to ensure we mark everything pertaining to
    // the object.
    if (val_type == VAL_FLOAT || val_type == VAL_STRING)
    {
      val->marked = true;
    }
    else if (val_type == VAL_LIST_NODE)
    {
      ListValue * list_val = (ListValue *) val;
      if (list_val->marked == false)
      {
        list_val->marked = true;

        mark(name, list_val->list_node.value);
        mark(name, list_val->list_node.next);
      }
    }
    else if (val_type == VAL_DICT_NODE)
    {
      DictValue * dic_val = (DictValue *) val;
      if (dic_val->marked == false)
      {
        dic_val->marked = true;

        mark(name, dic_val->dict_node.value);
        mark(name, dic_val->dict_node.key);
        mark(name, dic_val->dict_node.next);
      }
    }
}

/* This function implements a compacting mark and sweep garbage collection
and uses the helper function mark*/
...
int collect_garbage(void) {
    ...
    // Sweep em up
    while (curr < freeptr) {
        Value * curr_val = (Value *) curr;
        int data_size = curr_val->data_size;
        int value_size = sizeof(Value) + data_size;
        Reference ref = curr_val->ref;

        // Swept if not marked meaning it's not being used
        if (curr_val->marked == false)
        {
            ref = NULL_REF;
            ref_table[ref] = NULL;

        }
        // if it isn't marked then we will set "marked" back to false for next
        // marking phase
        else
        {
            curr_val->marked = false;
            // used memmove over memcopy since it is safer when dealing with
            // chunks of memory that may overlap albeit slower
            memmove((void *) dst, (const void *) curr, value_size);
            ref_table[ref] = (Value *) dst;
